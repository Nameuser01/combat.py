...
def menu():
	...
	print("'5' : Afficher toutes les informations sur les joueurs")
...
def modifie_name():
	...
	while(modif != 1 and modif != 2):
		print("La joueur que vous essayez de modifier n'existe pas")
		print("Veuillez réessayer")
		modif = int(input("> "))
	#On considère ici que $modif contient une valeur correcte
	if (modif == 1):
		print("Le nom actuel du joueur 1 est :", user1.get_name())
		new_name = "new_name"
		new_name = str(input("Quel est son nouveau nom ? \n> "))
...
class Player:
	...
	def change_name(self, new_name):
		assert isinstance (new_name, str) and len(new_name) >= 3
		new_name = self.name


print("Entrez les paramètres de ce programme")
name_user1 = str(input(""))
user1 = Player(name_user1)
user2 = Player(name_user2)
infos()
i = 0
token = False
while (token == False):
	...
	"""Condition qui permet de consulter les attributs du joueur choisis"""
	elif(choix == 3):
		modif = 0
	...
	"""Condition qui permet de modifier les attributs du joueur"""
	elif (choix == 4):
		modifie_name()
	elif (choix == 5):
		#Afficher les informations complètes sur les joueurs
		infos()
	else:
